          ...
          <div class="circle">
              <div class="avator">
                  <van-image round width="5rem" height="5rem" fit="cover" position="left" @click="gocenter"
                      :src="info.groupHead ? info.groupHead :'static/20230407163657.png'" />
                  <div class="text">
                      <p style="font-size: 20px;">群名称：{{ info.name }}</p>
...
lookGroup({groupId :route.query.group}).then(res => {
  console.log(res, 111);
  info.value = res.data
})
...
const agree = async()=> {
   let params = {
    ...
    joinUserId:route.query.id
   }
   try {
    const res = await agreeGroup(params)
     console.log(res);
